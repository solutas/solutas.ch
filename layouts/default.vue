<template>
  <v-app light>
    ...
    > <template v-slot:img="{ props }">
      ...
      <v-spacer />
       <v-btn class="mr-5 d-none d-md-inline" v-for="(item, i) in menu" :key="i"  router exact outlined style="font-weight:300;border:1px solid gray">
            {{item.title}}
        </v-btn>
      <v-btn icon @click.stop="rightDrawer = !rightDrawer" class="d-xs d-md-none">
...
export default {
  ...
  created: async function() {
    console.log('callings et masödklfj')
    await this.setMenu()
  }
}
</script>
